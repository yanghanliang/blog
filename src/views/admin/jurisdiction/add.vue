	...
	data() {
		let name = async (rule, value, callback) => {
			if (value) {
				...
				try {
					const { verification } = await this.$http.post('jurisdiction/verification', postData)
					if (verification) {
						callback()
					} else {
						callback(new Error('此权限名称已存在，请重新输入'))
					}
				} catch (e) {
					console.log(e)
				}
			...
			if (value) {
				...
				try {
					...
					if (verification) {
						...
						callback(new Error('此权限标识已存在，请重新输入'))
					}
				} catch (e) {
					console.log(e)
				}
		...
		return {
			...
			rules: {
				...
				identification: [
					{ validator: identification, trigger: 'change' }
				]
			},
