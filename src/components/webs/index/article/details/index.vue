        ...
        <div class="w">
            ...
            <div class="content clearfix scrollbar">
                ...
                <div class="right">
                    ...
                    <!-- 相关推荐 简写 rr -->
                    <ul class="rr mb10" v-if="recommend.data.length > 0">
                        <h3>您可能需要</h3>
...
export default {
	...
	methods: {
		async loadData () { // 获取文章详情数据
			try {
				const data = await this.$http.get(`articleDetails/${this.$route.params.articleId}?random=${Math.random()}`)
				this.articleData = data // 将数据赋值给 vue
				// 兼容移动端
				let bodyEle = null
